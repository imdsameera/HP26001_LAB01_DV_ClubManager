...
const NAV_GROUPS: NavGroup[] = [
  {
    ...
    items: [
      { id: "general",      label: "General Profile", icon: Building2, roles: ["SUPER_ADMIN"] },
      { id: "admin-access", label: "Admin Access",    icon: ShieldAlert, roles: ["SUPER_ADMIN", "ADMIN"] },
    ],
  },
  {
    label: "Preferences",
    items: [
      { id: "branding", label: "Branding",        icon: Palette, roles: ["SUPER_ADMIN"] },
      { id: "emails",   label: "Emails & Alerts", icon: Mail,    roles: ["SUPER_ADMIN"] },
      { id: "finance",  label: "Finance Config",  icon: DollarSign, roles: ["SUPER_ADMIN"] },
    ],
  },
];
...
function AdminAccessTab({ onChange }: { onChange: () => void }) {
  ...
  const [creationType, setCreationType] = useState<"promote" | "new">("promote");
  ...
  useEffect(() => { void fetchAdminsAndMembers(); }, [fetchAdminsAndMembers]);
  ...
  const canChangeRole = (admin: AdminUser) => {
    const selfId = session?.user && "id" in session.user ? (session.user as { id: string }).id : null;
    ...
    if (currentUserRole === "ADMIN" && !["SUPER_ADMIN", "ADMIN"].includes(admin.role)) return true; // Admin can change Secretary/Treasurer
    return false;
  };
      ...
      <Card title="Admin Users" description="Manage who has administrative access to this dashboard.">
        {loading ? (
          ...
          <>
                    ...
                    return (
                      <tr key={admin.id} className="group transition-colors hover:bg-blue-50/30">
                            ...
                            {canChange ? (
                              <>
                                <button
                                  ...
                                  onClick={() => setRoleDropdownOpenFor(roleDropdownOpenFor === admin.id ? null : admin.id)}
                                ...
                                >
                                  ...
                                  <ChevronDown size={10} className={`transition-transform ${roleDropdownOpenFor === admin.id ? "rotate-180" : ""}`} />
                                </button>
                                {roleDropdownOpenFor === admin.id && (
                                  <>
                                    <div className="fixed inset-0 z-30" onClick={() => setRoleDropdownOpenFor(null)} />
                                    <div className="absolute left-0 top-[calc(100%+4px)] z-40 min-w-[160px] overflow-hidden rounded-xl border border-gray-200 bg-white shadow-xl shadow-gray-200/50">
                                      {["SECRETARY", "TREASURER", "ADMIN", "SUPER_ADMIN"].map(r => {
                                        const isSingleton = r === "SECRETARY" || r === "TREASURER";
                                        const isTaken = isSingleton && admins.some(a => a.role === r && a.id !== admin.id);
                                        const isSelected = admin.role === r;
                                        // Admin users can only assign Secretary/Treasurer
                                        const isForbidden = currentUserRole === "ADMIN" && ["SUPER_ADMIN", "ADMIN"].includes(r);
                                        const isDisabled = isTaken || isForbidden;

                                        return (
                                          <div
                                            key={r}
                                            onClick={isDisabled || isSelected ? undefined : () => void handleRoleChange(admin.id, r)}
                                            className={`flex items-center justify-between px-4 py-2.5 text-sm transition-colors ${
                                              isDisabled ? "bg-gray-50/50 cursor-not-allowed" :
                                              isSelected ? "bg-blue-50/50 text-[#0066FF] font-medium cursor-default" :
                                              "text-slate-700 hover:bg-blue-50 cursor-pointer"
                                            }`}
                                          >
                                            <span className={isDisabled ? "text-gray-400 line-through" : ""}>{ROLE_LABELS[r]}</span>
                                            {isSelected && <Check size={14} className="text-[#0066FF]" />}
                                            {isTaken && <span className="text-[10px] font-bold tracking-wider text-gray-400 uppercase bg-gray-200 px-1.5 py-0.5 rounded">Active</span>}
                                            {isForbidden && !isTaken && <span className="text-[10px] font-bold tracking-wider text-gray-400 uppercase bg-gray-200 px-1.5 py-0.5 rounded">No Access</span>}
                                          </div>
                                        );
                                      })}
                                    </div>
                        ...
                        <td className="px-4 py-3.5 flex justify-end">
                          {(admin.role !== "SUPER_ADMIN" && !(currentUserRole === "ADMIN" && admin.role === "ADMIN")) && (
                            <button
                              type="button"
            ...
            {/* Add admin form */}
            {showForm ? (
              <form onSubmit={e => void handleAdd(e)} className="mt-4 space-y-4 rounded-xl border border-blue-100 bg-blue-50/40 p-5">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-semibold text-slate-800">Provision Admin Access</p>
                  <select 
                ...
                <div className="grid grid-cols-2 gap-4 relative z-20">
                  {creationType === "promote" ? (
                    <div className="col-span-2 relative">
                      <label className="mb-1 block text-xs font-medium text-gray-600">Select Active Member</label>
                      <div 
                      ...
                      >
                        <span className={form.email ? "text-slate-800 font-medium" : "text-gray-400"}>
                          {form.email ? `${form.name} (${form.email})` : "Search & Select Member..."}
                        </span>
                        <ChevronDown size={16} className={`text-gray-400 transition-transform ${memberDropdownOpen ? 'rotate-180' : ''}`} />
                      </div>

                      {memberDropdownOpen && (
                        <>
                          <div className="fixed inset-0 z-30" onClick={() => setMemberDropdownOpen(false)} />
                          <div className="absolute left-0 right-0 top-[calc(100%+4px)] z-40 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-xl shadow-gray-200/50">
                            ...
                            <div className="max-h-56 overflow-y-auto w-full">
                              {(() => {
                                const avail = members.filter(m => m.email && !admins.some(a => a.email === m.email));
                                const filtered = avail.filter(m => 
                                  m.name.toLowerCase().includes(memberSearch.toLowerCase()) || 
                                  m.email.toLowerCase().includes(memberSearch.toLowerCase())
                                );
                                if (avail.length === 0) return <div className="p-3 text-center text-sm text-gray-500">No active members available.</div>;
                                if (filtered.length === 0) return <div className="p-3 text-center text-sm text-gray-500">No matches found.</div>;
                                return filtered.map(m => (
                                  <div
                                    key={m.id}
                                    onClick={() => {
                                      setForm(p => ({ ...p, email: m.email, name: m.name }));
                                  ...
                                  >
                                    <div className="flex flex-col">
                                      <span>{m.name}</span>
                                      <span className={`text-[11px] ${form.email === m.email ? 'text-blue-500' : 'text-gray-400'}`}>{m.email}</span>
                                    </div>
                                    {form.email === m.email && <Check size={16} className="text-[#0066FF]" />}
                                  </div>
                                ));
                              })()}
                            </div>
                  ...
                  <div className="relative">
                    ...
                    >
                      <span className="text-slate-800 font-medium">{ROLE_LABELS[form.role] ?? form.role}</span>
                    ...
                    {roleDropdownOpen && (
                      <>
                        ...
                        <div className="absolute left-0 right-0 top-[calc(100%+4px)] z-40 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-xl shadow-gray-200/50">
                          {["SECRETARY", "TREASURER", "ADMIN", "SUPER_ADMIN"].map((r) => {
                            const isSingleton = r === "SECRETARY" || r === "TREASURER";
                            const isTaken = isSingleton && admins.some(a => a.role === r);
                            const isSelected = form.role === r;

                            if (isTaken) {
                              return (
                                <div key={r} className="flex flex-col px-4 py-2.5 text-sm bg-gray-50/50 cursor-not-allowed">
                                  <div className="flex items-center justify-between">
                                    <span className="text-gray-400 line-through">{ROLE_LABELS[r]}</span>
                                    <span className="text-[10px] font-bold tracking-wider text-gray-400 uppercase bg-gray-200 px-1.5 py-0.5 rounded">Active</span>
                                  </div>
                                </div>
                              );
                            }
                            ...
                            return (
                              ...
                              >
                                ...
                                {isSelected && <Check size={16} className="text-[#0066FF]" />}
                              </div>
                            );
                          })}
                        </div>
                  ...
                  <div>
                    <label className="mb-1 block text-xs font-medium text-gray-600">
                      {creationType === "promote" ? "New Dashboard Password" : "Password"}
                    </label>
                    <input required type="password" value={form.password} onChange={e => setForm(p => ({ ...p, password: e.target.value }))}
                      className={INPUT_CLS + " py-2.5"} placeholder="Min 8 characters" />
                  </div>
                </div>
...
function SettingsPageContent() {
  ...
  useEffect(() => {
    if (userRole) {
      const allAllowedTabs = filteredGroups.flatMap(g => g.items.map(i => i.id));
